...
func (t *TaskService) SaveTask(ctx context.Context, msg *task.TaskReqMessage) (*task.TaskMessage, error) {
	...
	//添加动态
	err = createProjectLog(t.projectLogRepo, ts.ProjectCode, ts.Id, ts.Name, ts.AssignTo, "create", "task")
	if err != nil {
...
func (t *TaskService) MyTaskList(ctx context.Context, msg *task.TaskReqMessage) (*task.MyTaskListResponse, error) {
	...
	go func() {
		mList, _ := rpc.LoginServiceClient.FindMemInfoByIds(ctx, &login.UserMessage{MIds: mids})
		mListChan <- mList
	}()
	pList := <-pListChan
	projectMap := data.ToProjectMap(pList)
...
func (t *TaskService) CreateComment(ctx context.Context, msg *task.TaskReqMessage) (*task.CreateCommentResponse, error) {
	...
	err = t.projectLogRepo.SaveProjectLog(pl)
	if err != nil {
		zap.L().Error("project task CreateComment fileRepo.SaveProjectLog error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	return &task.CreateCommentResponse{}, nil
}

func (t *TaskService) TaskDone(ctx context.Context, msg *task.TaskReqMessage) (*task.TaskDoneResponse, error) {
	taskCode := encrypts.DecryptNoErr(msg.TaskCode)
	err := t.taskRepo.TaskDone(context.Background(), taskCode, msg.Done)
	if err != nil {
		zap.L().Error("project TaskDone error", zap.Error(err))
		return nil, errs.GrpcError(model.DBError)
	}
	return &task.TaskDoneResponse{}, nil
}
