...
import (
	...
	"gwh.com/project-api/pkg/model/tasks"
	common "gwh.com/project-common"
	"gwh.com/project-common/errs"
	"gwh.com/project-common/fs"
...
func (t *HandlerTask) taskMemberList(c *gin.Context) {
	...
	memberResp, err := rpc.TaskServiceClient.MemberProjectList(ctx, msg)
	if err != nil {
		code, msg := errs.ParseGrpcError(err)
		c.JSON(http.StatusOK, result.Fail(code, msg))
	}
...
func (t *HandlerTask) taskList(c *gin.Context) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Second)
	defer cancel()
	list, err := rpc.TaskServiceClient.TaskList(ctx, &task.TaskReqMessage{StageCode: stageCode})
	if err != nil {
		code, msg := errs.ParseGrpcError(err)
		c.JSON(http.StatusOK, result.Fail(code, msg))
	}
